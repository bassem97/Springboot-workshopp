...
public class StudentController {
    ...
    ResponseEntity<Response<AttributeType>> fields(){
        try {
            return ResponseEntity.ok().body(new Response<>(true, HttpStatus.OK.value(), "Fields retrieved successfully", studentService.getStudentFields()));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(new Response<>(false, HttpStatus.BAD_REQUEST.value(), e.getMessage(), null));
        }
    }
